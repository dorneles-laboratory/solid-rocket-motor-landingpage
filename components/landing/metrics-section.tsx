...
const metrics = [
  ...
  },
  {
    ...
    sublabel: "grão BATES, 3 segmentos",
  },
];
    ...
    <section ref={sectionRef} className="relative py-32 lg:py-40 overflow-hidden">
      ...
      <div className="relative z-10 max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="grid lg:grid-cols-12 gap-8 mb-16 lg:mb-24">
          <div className="lg:col-span-8 lg:col-start-1">
            <div className="flex items-center gap-4 mb-6">
              <span className="flex items-center gap-2 px-3 py-1 bg-[#ffa500]/10 text-[#ffa500] text-xs font-mono">
            ...
            <h2 className={`text-6xl md:text-7xl lg:text-[120px] font-display tracking-tight leading-[0.95] transition-all duration-1000 ${
              isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            }`}>
              Análise
              <br />
              <span className="text-muted-foreground">termo-balística.</span>
            </h2>
          </div>
        </div>

        {/* Thrust/pressure chart */}
        <div className={`w-full mb-12 rounded-xl border border-foreground/10 overflow-hidden bg-card transition-all duration-1000 delay-200 ${
          isVisible ? "opacity-100" : "opacity-0"
        }`}>
          <img
            src="/images/thrust-curve-dashboard.png"
            alt="Curvas de empuxo e pressão versus tempo"
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          <div className={`lg:col-span-1 bg-card border border-foreground/10 rounded-lg p-10 lg:p-14 transition-all duration-700 ${
            ...
            <div className="mb-6">
              <DotGraph color="orange" height={36} freq1={0.28} freq2={0.09} freqT={0.5} speed={0.018} baseline={0.35} amplitude={0.55} />
            </div>
          ...
          {metrics.slice(1).map((metric, index) => (
            <div
              key={metric.label}
              className={`bg-card border border-foreground/10 rounded-lg p-8 flex flex-col items-start justify-between gap-6 transition-all duration-700 ${
                isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"
              }`}
        ...
        {/* Bottom ticker — propellants & materials */}
        <div className={`mt-16 pt-8 border-t border-foreground/10 flex flex-wrap items-center gap-x-12 gap-y-4 text-sm font-mono text-muted-foreground transition-all duration-1000 delay-500 ${
          ...
          <span>APCP</span>
          <span>Aço 1020</span>
          <span className="text-foreground">+ catálogo personalizável</span>
        </div>
      </div>
    </section>
  );
